...
⚠ `pytest` 를 돌릴 때는 이걸 내리는 편이 낫다 — 실포트(50001/6550/1460)를 잡고 있기 때문이다.
다만 **전체 스위트가 간헐적으로 멈추는 것은 이것 때문이 아니다.** 가짜 장치가 떠 있을 때 두 번,
없을 때 한 번 겪었다(2026-09-07~08). 15분이 넘도록 무반응이고 CPU 는 거의 0 이라 느린 것처럼
보이지도 않는다. 죽이고 다시 돌리면 26초에 끝난다. 원인 미규명 — TASKS BUG-TEST-SUITE-HANG.
